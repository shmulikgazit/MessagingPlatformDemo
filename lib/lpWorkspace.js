...
function registerConversationHandlers(conv) {
  ...
  conv.on('message-delivery-event', (messages) => {
    broadcast('message-delivery-event', { conversationId: conv.conversationId, messages });
  });
  ...
  conv.on('transfer-skill', () => {
    broadcast('transfer-skill', { conversationId: conv.conversationId, skill: conv.skill });
  });
...
function listConversations() {
  return Array.from(conversations.values()).map((c) => snapshotConversation(c));
}
