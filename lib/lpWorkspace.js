...
const rings = new Map();
/**
 * Routing removed this conversation from the agent's workload (leave, back-to-queue, skill/agent transfer).
 * SDK may retain the Conversation in memory; sync/list must exclude these ids until a ring accept or reconnect clears state.
 */
...
/** User profile labels from SDK Participant.getUserProfile(); cached across conversations in this WS session. */
let participantProfileCache = new Map();
let participantProfileInflight = new Map();

function participantProfileCacheKey(p) {
  if (!p) {
    return null;
  }
  try {
    if (typeof p.isConsumer === 'function' && p.isConsumer()) {
      return p.userId != null ? `c:${String(p.userId)}` : null;
    }
  } catch (_) {
    /* ignore */
  }
  if (p.agentId != null && String(p.agentId).trim()) {
    return `a:${String(p.agentId)}`;
  }
  if (p.userId != null && String(p.userId).trim()) {
    return `u:${String(p.userId)}`;
  }
  return null;
}

function displayNameFromProfileFields(profile) {
...
async function serializeParticipantWithProfile(p) {
  const base = serializeParticipant(p);
  if (!p) return base;
  const key = participantProfileCacheKey(p);
  if (!key) {
    return base;
  }
  if (participantProfileCache.has(key)) {
    const extras = participantProfileCache.get(key);
    const out = { ...base };
    if (extras.profile) out.profile = extras.profile;
    if (extras.displayName) out.displayName = extras.displayName;
    return extras.profile || extras.displayName ? out : base;
  }
  let loader = participantProfileInflight.get(key);
  if (!loader) {
    loader = p
      .getUserProfile()
      .then((raw) => {
        const profile =
          raw && typeof raw === 'object'
            ...
            : {};
        const displayName = displayNameFromProfileFields(profile);
        participantProfileCache.set(key, { profile, displayName: displayName || null });
      })
      .catch(() => {
        participantProfileCache.set(key, {});
      })
      .finally(() => {
        participantProfileInflight.delete(key);
      });
    participantProfileInflight.set(key, loader);
  }
  await loader;
  const extras = participantProfileCache.get(key) || {};
  const out = { ...base };
...
async function serializeMessageEnriched(m) {
  return {
    sequence: m.sequence,
    time: m.time,
    body: m.body,
    contentType: m.contentType,
    messageAudience: m.messageAudience,
    participant: await serializeParticipantWithProfile(m.participant),
    sentByCurrentUser: typeof m.sentByCurrentUser === 'function' ? m.sentByCurrentUser() : false,
    hasConsumerRead: typeof m.hasConsumerRead === 'function' ? m.hasConsumerRead() : false,
    hasAssignedAgentRead: typeof m.hasAssignedAgentRead === 'function' ? m.hasAssignedAgentRead() : false,
  };
...
async function snapshotConversationEnriched(conv) {
  const dPrimary = conv.openDialog || conv.getDialog(lpm.DialogType.MAIN);
  /** Ensures MAIN dialog publishes are loaded into `dialog.messages` (SDK may cache only recent lines until hydrated). */
  if (dPrimary && typeof dPrimary.getAllPublishEvents === 'function') {
    try {
      await dPrimary.getAllPublishEvents();
    } catch (e) {
      // eslint-disable-next-line no-console
      console.warn('[lpWorkspace] getAllPublishEvents failed', e.message || e);
    }
  }
  const d = conv.openDialog;
  const skill = conv.skill ? { skillId: conv.skill.skillId } : null;
  ...
  let participants = [];
  if (d) {
    messages = await Promise.all(d.messages.map((m) => serializeMessageEnriched(m)));
    participants = await Promise.all(d.participants.map((p) => serializeParticipantWithProfile(p)));
  }
  return {
    conversationId: conv.conversationId,
    stage: conv.stage,
...
async function disconnect() {
  ...
  participantProfileInflight.clear();
  await conn.close();
...
async function getConversationSnapshot(id) {
  const conv = getConversationOrThrow(id);
  return snapshotConversationEnriched(conv);
}
...
async function joinConversation(id, role) {
  ...
  await conv.join(r);
  return await getConversationSnapshot(id);
}
...
async function transferConversation(id, { skillId, agentId } = {}) {
  ...
  const snapshot = await snapshotConversationEnriched(conv);
...
/**
 * Optional comma-separated skill ids for the agent workspace transfer dropdown (demo / routing shortcuts).
 * Each entry is `skillId` or `skillId:Label` (label shown after the id).
 * Example: TRANSFER_SKILL_IDS=111:Billing,222,SalesTeamSkillId
 */
function getTransferSkillsCatalog() {
  const raw = process.env.TRANSFER_SKILL_IDS || '';
  const out = [];
  const seen = new Set();
  for (const part of raw.split(',')) {
    ...
    if (colon === -1) {
      ...
      skillId = s.slice(0, colon).trim();
      label = s.slice(colon + 1).trim() || skillId;
    }
    if (!skillId || seen.has(skillId)) continue;
    seen.add(skillId);
    out.push({ skillId, label });
  }
  return out;
}

/**
 * Skills catalog for transfer UI: merges Skills API with TRANSFER_SKILL_IDS when connected.
 * @param {string|null|undefined} conversationSkillId — skill id from snapshot when fetching transfer targets for current skill.
 */
async function resolveTransferSkillsCatalog(conversationSkillId) {
  const envSkills = getTransferSkillsCatalog();
  if (!connection) {
    return {
      skills: envSkills,
      source: ['env'],
      ...
      apiError:
        envSkills.length === 0
          ? 'Not connected — open a session to load skills from the Skills API (or set TRANSFER_SKILL_IDS).'
          : 'Not connected — showing TRANSFER_SKILL_IDS only; Skills API loads after connect.',
    };
  }
  try {
    const r = await skillsApi.resolveTransferSkillsFromApi(connection, conversationSkillId, envSkills);
    return {
      skills: r.skills,
      source: r.source,
      strategy: r.strategy,
    ...
    return {
      skills: envSkills,
      source: ['env'],
      strategy: 'env_fallback_error',
      apiError: detail.slice(0, 400),
    };
  }
}
...
function getConnectionHints() {
  ...
  return {
    ...
    participantRoles,
    chatStates,
    getTransferSkillsCatalog,
    resolveTransferSkillsCatalog,
  };
}
